...
const fs = require('fs');
const { availableProviders, allProvidersCatalog, askProvider, PROVIDERS } = require('./lib/providers');
...
app.get('/api/providers', (req, res) => {
  res.json({ providers: availableProviders(), allProviders: allProvidersCatalog() });
});

// Reports only whether each expected env var is present (never its value) —
// the fastest way to confirm whether a "not configured" state is a real
// missing/misnamed env var vs. a stale frontend. Safe to leave public: a
// boolean presence flag reveals nothing an attacker could use.
