(function () {
  ...
  async function loadProvidersAndTools() {
    try {
      ...
      const allProviders = provData.allProviders || [];

      if (!allProviders.length) {
        providerSelect.innerHTML = '<option>No providers registered</option>';
        runBtn.disabled = true;
      } else {
        providerSelect.innerHTML = allProviders.map((p) => `
          <option value="${p.id}" ${p.configured ? '' : 'disabled'}>${escapeHtml(p.name)}${p.configured ? '' : ' — needs API key'}</option>
        `).join('');
        const firstConfigured = allProviders.find((p) => p.configured);
        if (firstConfigured) {
          providerSelect.value = firstConfigured.id;
          runBtn.disabled = false;
        } else {
          runBtn.disabled = true;
          statusHint.textContent = 'No provider is configured yet — add at least one API key on the server to start.';
        }
      }
