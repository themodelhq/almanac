...
**If the frontend still shows a tool as "off" after adding the matching env
var**, work through these in order before assuming there's a code bug — the
`isAvailable()` check for every tool just reads `process.env` fresh on every
request, so it's almost always one of the following instead:
...
4. **Test the endpoint directly** — open `https://your-app.onrender.com/api/tools`
   in a browser tab. That bypasses the frontend and any caching entirely; if it
   shows the right values there but the page still looks wrong, it's a stale
   page load (hard refresh with Ctrl+Shift+R).
